...
"!
syntax region cIncludeFold start="^\s*\#include\s" skip="\(\n\s*\)\+\#include\s" end="$" keepend contains=cInclude,cIncluded,@Spell fold
syntax region cDefine start="^\s*\#define\s" skip="\(\n\s*\)\+\#define\s" end="$" keepend contains=cDefine,@Spell fold
syntax region cCommentL start=+^\s*\/\/+ skip="\(\n\s*\)\+\/\/" end=+$+ keepend contains=doxygenTODO,doxygenOtherTODO,@Spell fold
