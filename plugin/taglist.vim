...
function! s:Tlist_Parse_Tagline(tag_line)
    ...
    " ypguo add: The tag_name is parsed from the original buf and
    " encoded with original buffer's &fenc, so convert it to new
    " buffer's &enc if they are different.
...
function! s:Tlist_Process_File(filename, ftype)
    ...
    "-             \ && (&shell =~ 'cmd.exe')
    " ypguo disable it: Windows donen't work well with Chinese in batch file,
    " I didn't see the problem described in the following.
    if 0
    ...
    " ypguo add: win32 cmd(shell) need 'cpxxx' characters, so convert it
    " before pass the parameters, if chcp doesn't work, please set it maually.
    if executable("chcp")
...
function! s:Tlist_Window_Jump_To_Tag(win_ctrl)
    ...
    if tidx != 0
        ...
        " is the same encoding as the original buffer's &fenc
        if(&enc!=getbufvar("#", "&fenc"))
...
function! s:Tlist_Refresh_Folds()
    ...
    " @see http://explorerstreet.blogspot.com/2009/07/my-vim-setup.html
    if g:Tlist_Show_One_File
        return
    endif
