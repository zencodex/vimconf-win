...
syntax region javaImport start="^\s*import\s" skip="\(\n\s*\)\+import\s" end=+$+ keepend contains=javaExternal,javaAnnotation,@Spell fold
syntax region javaLineComment start=+^\s*\/\/+ skip="\(\n\s*\)\+\/\/" end=+$+ keepend contains=doxygenTODO,doxygenOtherTODO,@Spell fold
