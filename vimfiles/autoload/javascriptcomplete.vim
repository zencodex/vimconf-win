...
" URL: http://vim.cybermirror.org/runtime/autoload/javascriptcomplete.vim
	...
	if shortcontext =~ '\.$'
		...
		if len(object) > 0
			...
			if decl_line > 0
				...
				let decl_lineRegExp = search('var\s*'.object.'\s*=\s*\/', 'bn')
                let decl_lineString = search('var\s*'.object.'\s*=\s*["'."']", 'bn')
                let decl_lineArray = search('var\s*'.object.'\s*=\s*\[', 'bn')
                let decl_lineObject = search('var\s*'.object.'\s*=\s*{', 'bn')
				if decl_lineRegExp > 0
					let object_type = 'RegExp'
                elseif decl_lineString > 0
                    let object_type = 'String'
                elseif decl_lineArray > 0
                    let object_type = 'Array'
                elseif decl_lineObject > 0
